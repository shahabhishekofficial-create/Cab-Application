...
alter table public.sessions
  add column if not exists close_client_transaction_id uuid;

create unique index if not exists uq_sessions_close_client_transaction_id
  on public.sessions(close_client_transaction_id)
  where close_client_transaction_id is not null;
...
begin
  ...
  if found then
    return jsonb_build_object('sessionId', s.id, 'status', 'CLOSED', 'idempotent', true,
      'runningKm', coalesce(s.close_odometer, 0) - s.start_odometer);
  end if;
  ...
  update sessions set
    status = 'CLOSED', close_client_transaction_id = p_client_transaction_id,
    closed_at = p_closed_at, close_odometer = p_close_odometer,
    close_lat = p_close_lat, close_lng = p_close_lng,
    close_accuracy_m = p_close_accuracy_m, close_gps_at = p_close_gps_at,
    close_odometer_file_id = p_close_odometer_file_id, notes = p_notes, updated_at = now()
  where id = s.id;

  update vehicles set current_odometer = greatest(current_odometer, p_close_odometer), updated_at = now()
  where id = s.vehicle_id;

  insert into reconciliations (session_id, reported_trip_count, reported_income, system_trip_count, system_income,
    trip_count_difference, income_difference, running_km, completed_trip_km, cancelled_no_show_km, unallocated_km, status, notes)
  values (s.id, p_reported_trip_count, p_reported_income, system_trip_count, system_income,
    trip_count_difference, income_difference, running_km, completed_km, cancelled_km, unallocated_km, reconciliation_status, p_notes)
  on conflict (session_id) do update set
    reported_trip_count = excluded.reported_trip_count, reported_income = excluded.reported_income,
    system_trip_count = excluded.system_trip_count, system_income = excluded.system_income,
    trip_count_difference = excluded.trip_count_difference, income_difference = excluded.income_difference,
    running_km = excluded.running_km, completed_trip_km = excluded.completed_trip_km,
    cancelled_no_show_km = excluded.cancelled_no_show_km, unallocated_km = excluded.unallocated_km,
  ...
  return jsonb_build_object('sessionId', s.id, 'status', 'CLOSED', 'idempotent', false,
    'reconciliationStatus', reconciliation_status, 'runningKm', running_km,
    'systemTripCount', system_trip_count, 'systemIncome', system_income, 'unallocatedKm', unallocated_km);
end;
$$;
