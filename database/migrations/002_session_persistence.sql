...
begin
  ...
  if not exists (select 1 from driver_vehicle_assignments where driver_id = p_driver_id and vehicle_id = p_vehicle_id and assigned_from <= p_started_at and (assigned_to is null or assigned_to > p_started_at)) then
  ...
  insert into sessions (client_transaction_id, driver_id, vehicle_id, device_id, session_date, status, started_at, start_odometer, start_lat, start_lng, start_accuracy_m, start_gps_at, start_odometer_file_id, notes)
  values (p_client_transaction_id, p_driver_id, p_vehicle_id, p_device_id, (p_started_at at time zone 'Asia/Kolkata')::date, 'OPEN', p_started_at, p_start_odometer, p_start_lat, p_start_lng, p_start_accuracy_m, p_start_gps_at, p_start_odometer_file_id, p_notes)
...
exception
  when unique_violation then
    if sqlerrm like '%uq_driver_vehicle_open_session%' then raise exception using errcode = 'P0001', message = 'SESSION_ALREADY_OPEN'; end if;
    raise;
end;
$$;
...
begin
  ...
  select count(*), coalesce(sum(gross_fare), 0), coalesce(sum(case when status = 'COMPLETED' and end_odometer is not null then end_odometer - start_odometer else 0 end), 0), coalesce(sum(case when status <> 'COMPLETED' and end_odometer is not null then end_odometer - start_odometer else 0 end), 0)
  into system_trip_count, system_income, completed_km, cancelled_km from trips where session_id = s.id;
  ...
  update sessions set status = 'CLOSED', closed_at = p_closed_at, close_odometer = p_close_odometer, close_lat = p_close_lat, close_lng = p_close_lng, close_accuracy_m = p_close_accuracy_m, close_gps_at = p_close_gps_at, close_odometer_file_id = p_close_odometer_file_id, notes = p_notes, updated_at = now() where id = s.id;
  ...
  if reconciliation_status = 'CRITICAL' then
    insert into exceptions(entity_type, entity_id, code, severity, message) values ('SESSION', s.id, 'SESSION_RECONCILIATION_CRITICAL', 'CRITICAL', 'Session close produced negative running or unallocated kilometres');
  elsif reconciliation_status = 'REVIEW' then
    insert into exceptions(entity_type, entity_id, code, severity, message) values ('SESSION', s.id, 'SESSION_RECONCILIATION_REVIEW', 'WARNING', 'Reported session totals differ from system totals');
  end if;
  return jsonb_build_object('sessionId', s.id, 'status', 'CLOSED', 'reconciliationStatus', reconciliation_status, 'runningKm', running_km, 'systemTripCount', system_trip_count, 'systemIncome', system_income, 'unallocatedKm', unallocated_km);
end;
$$;
...
grant execute on function start_session(uuid, uuid, uuid, uuid, timestamptz, numeric, numeric, numeric, numeric, timestamptz, uuid, text) to service_role;
grant execute on function close_session(uuid, uuid, uuid, timestamptz, numeric, numeric, numeric, numeric, timestamptz, uuid, integer, numeric, text) to service_role;
