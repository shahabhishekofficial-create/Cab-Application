...
-- The Android client creates a stable session UUID locally and all later
-- transactions reference it. The original RPC generated a different UUID,
-- which made subsequent TRIP/FUEL/EXPENSE/SESSION_CLOSE records point at a
-- session that did not exist. Make the client session UUID the server row ID.
drop function if exists public.start_session(uuid,uuid,uuid,uuid,timestamptz,numeric,numeric,numeric,numeric,timestamptz,uuid,text);
...
revoke all on function public.start_session(uuid,uuid,uuid,uuid,uuid,timestamptz,numeric,numeric,numeric,numeric,timestamptz,uuid,text) from public;
