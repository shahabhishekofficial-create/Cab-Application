-- The API validates the Supabase access token first, then passes the authenticated
-- user id explicitly because the service-role DB client does not carry auth.uid().
