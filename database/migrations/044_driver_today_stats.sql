...
declare v_driver_id uuid; v_display_name text; v_vehicle_id uuid; v_registration text; v_trips integer; v_distance numeric; v_earnings numeric; v_fuel numeric; v_hours numeric;
begin
 ...
 select v.id,v.registration_number into v_vehicle_id,v_registration from public.driver_vehicle_assignments a join public.vehicles v on v.id=a.vehicle_id where a.driver_id=v_driver_id and a.assigned_to is null and v.status='ACTIVE' order by a.assigned_from desc limit 1;
 ...
 return jsonb_build_object('userId',p_user_id,'driverId',v_driver_id,'displayName',v_display_name,'vehicleId',v_vehicle_id,'registrationNumber',v_registration,'today',jsonb_build_object('trips',v_trips,'distanceKm',round(v_distance,2),'earnings',round(v_earnings,2),'fuelSpend',round(v_fuel,2),'hoursActive',round(v_hours,2)));
end; $$;
