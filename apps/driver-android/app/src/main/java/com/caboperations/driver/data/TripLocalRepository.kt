...
class TripLocalRepository(private val context: android.content.Context) {
 ...
 suspend fun startTrip(sessionId:String,driverId:String,vehicleId:String,startOdometer:Double,location:LocationSnapshot?,pickup:String?=null,platformId:String?=null,notes:String?=null):String{
  ...
  val id=UUID.randomUUID().toString(); val startedAt=gpsTime(location) ?: Instant.now().toString()
  val payload=buildJsonObject{put("clientTransactionId",id);put("sessionId",sessionId);put("driverId",driverId);put("vehicleId",vehicleId);put("startedAt",startedAt);put("startOdometer",startOdometer);put("grossFare",0);put("additionalCharges",0);put("status","IN_PROGRESS");platformId?.let{put("platformId",it)};pickup?.let{put("pickup",it)};location?.takeIf{it.isUsable()}?.let{put("latitude",it.latitude);put("longitude",it.longitude);put("gpsAccuracyM",it.accuracyMeters.toDouble());put("gpsAt",startedAt)};notes?.let{put("notes",it)}}.toString()
 ...
 suspend fun endTrip(tripClientTransactionId:String,sessionId:String,driverId:String,vehicleId:String,endOdometer:Double,grossFare:Double,location:LocationSnapshot?,paymentMethod:String?=null,additionalCharges:Double=0.0,status:String="COMPLETED",pickup:String?=null,dropoff:String?=null,platformId:String?=null,notes:String?=null):String{
  require(endOdometer>=0){"Ending odometer cannot be negative"};require(grossFare>=0){"Fare cannot be negative"};require(additionalCharges>=0){"Additional charges cannot be negative"}
  ...
  val id=UUID.randomUUID().toString();val endedAt=gpsTime(location) ?: Instant.now().toString();val payload=buildJsonObject{put("clientTransactionId",id);put("tripClientTransactionId",tripClientTransactionId);put("sessionId",sessionId);put("driverId",driverId);put("vehicleId",vehicleId);put("endedAt",endedAt);put("endOdometer",endOdometer);put("grossFare",grossFare);paymentMethod?.let{put("paymentMethod",it)};put("additionalCharges",additionalCharges);put("status",status);platformId?.let{put("platformId",it)};pickup?.let{put("pickup",it)};dropoff?.let{put("dropoff",it)};location?.takeIf{it.isUsable()}?.let{put("latitude",it.latitude);put("longitude",it.longitude);put("gpsAccuracyM",it.accuracyMeters.toDouble());put("gpsAt",endedAt)};notes?.let{put("notes",it)}}.toString()
