...
class SyncWorker(appContext:Context,params:WorkerParameters):CoroutineWorker(appContext,params){
 override suspend fun doWork():Result{
  ...
  for(item in pending){val json=runCatching{Json.parseToJsonElement(item.payloadJson).jsonObject}.getOrNull();if(json==null){dao.markFailed(item.clientTransactionId,"INVALID_LOCAL_PAYLOAD");continue};val driverId=json["driverId"]?.jsonPrimitive?.content;val vehicleId=json["vehicleId"]?.jsonPrimitive?.content;var authRetried=false
   fun path():String?=when(item.type){TYPE_SESSION_START->"/v1/sessions";TYPE_SESSION_CLOSE->json["sessionId"]?.jsonPrimitive?.content?.let{"/v1/sessions/$it/close"};TYPE_TRIP,TYPE_TRIP_START->"/v1/trips";TYPE_TRIP_END->"/v1/trips/end";TYPE_FUEL->"/v1/fuel";TYPE_EXPENSE->"/v1/expenses";else->null}
   fun body():String=if(item.type==TYPE_TRIP_START){val clean=json.toMutableMap();clean.remove("status");Json.encodeToString(JsonObject.serializer(),JsonObject(clean+mapOf("status" to kotlinx.serialization.json.JsonPrimitive("IN_PROGRESS"))))}else item.payloadJson
