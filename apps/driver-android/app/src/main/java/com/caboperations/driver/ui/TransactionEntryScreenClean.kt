...
fun TransactionEntryScreenClean(type:String,sessionId:String,driverId:String,vehicleId:String,onSaved:(String)->Unit,onCancel:()->Unit){
    ...
    var activeTrip by remember{mutableStateOf<LocalTrip?>(null)}; var startedAt by remember{mutableStateOf<String?>(null)}; var startPlatformId by remember{mutableStateOf<String?>(null)}; var odo by remember{mutableStateOf("")}; var endOdo by remember{mutableStateOf("")}; var fare by remember{mutableStateOf("")}; var charges by remember{mutableStateOf("0")}; var payment by remember{mutableStateOf("UPI")}; var pickup by remember{mutableStateOf("")}; var drop by remember{mutableStateOf("")}; var amount by remember{mutableStateOf("")}; var notes by remember{mutableStateOf("")}; var fuelType by remember{mutableStateOf("CNG")}; var qty by remember{mutableStateOf("")}; var rate by remember{mutableStateOf("")}; var unit by remember{mutableStateOf("KG")}; var busy by remember{mutableStateOf(false)}; var error by remember{mutableStateOf("")}; var location by remember{mutableStateOf<LocationSnapshot?>(null)}; var menu by remember{mutableStateOf(false)}; var platformMenu by remember{mutableStateOf(false)}; var platforms by remember{mutableStateOf<List<PlatformOption>>(emptyList())}; var platformId by remember{mutableStateOf<String?>(null)}
    val isTrip=type=="TRIP"; val isFuel=type=="FUEL"; val q=qty.toDoubleOrNull(); val r=rate.toDoubleOrNull(); val total=if(q!=null&&r!=null&&q>0&&r>=0)q*r else null; val selectedPlatform=platforms.firstOrNull{it.id==(platformId ?: startPlatformId)}
    ...
    LaunchedEffect(Unit){
        capture()
        withContext(Dispatchers.IO){auth.session()?.let{s->platforms=PlatformRepository(BuildConfig.API_BASE_URL,s.accessToken).load().getOrDefault(emptyList())}}
