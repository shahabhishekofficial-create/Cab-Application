...
@Dao interface PendingTransactionDao {
 ...
 @Query("SELECT * FROM pending_transactions WHERE synced = 0 AND (attempts < ${SyncPolicy.MAX_RETRY_ATTEMPTS} OR lastError = 'TRANSACTION_FAILED') ORDER BY CASE type WHEN 'SESSION_START' THEN 0 WHEN 'TRIP_START' THEN 10 WHEN 'TRIP_END' THEN 11 WHEN 'TRIP' THEN 12 WHEN 'FUEL' THEN 12 WHEN 'EXPENSE' THEN 12 WHEN 'SESSION_CLOSE' THEN 20 WHEN 'FILE_UPLOAD' THEN 30 ELSE 40 END, createdAt LIMIT ${SyncPolicy.MAX_BATCH_SIZE}") suspend fun pending(): List<PendingTransaction>
