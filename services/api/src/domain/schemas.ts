...
export const startSessionSchema = z.object({
  ...
  startLat: z.number().gte(-90).lte(90), startLng: z.number().gte(-180).lte(180), startAccuracyM: z.number().finite().nonnegative().max(50), startGpsAt: z.string().datetime(),
...
export const tripSchema = z.object({
  ...
  additionalCharges: z.number().finite().nonnegative().default(0), status: z.enum(['COMPLETED','CANCELLED_BY_CUSTOMER','CANCELLED_BY_DRIVER','CUSTOMER_NO_SHOW']),
  ...gpsFields, gpsAt: z.string().datetime(), notes: z.string().max(2000).nullable().optional(),
  ...
  if (Math.abs(new Date(value.gpsAt).getTime() - new Date(value.startedAt).getTime()) > 10 * 60 * 1000) ctx.addIssue({ code: 'custom', path: ['gpsAt'], message: 'GPS timestamp is stale' });
});
...
export const fuelSchema = z.object({
  ...
  latitude: z.number().gte(-90).lte(90), longitude: z.number().gte(-180).lte(180), gpsAccuracyM: z.number().finite().nonnegative().max(50), recordedAt: z.string().datetime(), notes: z.string().max(2000).nullable().optional(),
}).superRefine((value, ctx) => {
  const expectedAmount = value.quantity * value.rate;
  if (Math.abs(value.amount - expectedAmount) > 0.01) ctx.addIssue({ code: 'custom', path: ['amount'], message: 'Fuel amount must equal quantity × rate within ₹0.01' });
});
...
export const expenseSchema = z.object({
  ...
  odometer: z.number().finite().nonnegative().nullable().optional(), latitude: z.number().gte(-90).lte(90), longitude: z.number().gte(-180).lte(180), gpsAccuracyM: z.number().finite().nonnegative().max(50), recordedAt: z.string().datetime(), notes: z.string().max(2000).nullable().optional(),
});
...
export const closeSessionSchema = z.object({
  ...
  closeLat: z.number().gte(-90).lte(90), closeLng: z.number().gte(-180).lte(180), closeAccuracyM: z.number().finite().nonnegative().max(50), closeGpsAt: z.string().datetime(), closeOdometerFileId: uuidSchema.nullable().optional(),
