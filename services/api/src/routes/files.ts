...
import { isAllowedUploadMimeType, isSafeObjectPath } from '../domain/file-policy.js';
...
export async function registerFileRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/v1/files', async (request, reply) => {
    ...
    const parsed = metadataSchema.safeParse({ fileId: request.headers['x-file-id'], objectPath: request.headers['x-object-path'], mimeType: request.headers['content-type'], capturedAt: request.headers['x-captured-at'] || null });
    ...
    if (existing && (existing.object_path !== parsed.data.objectPath || existing.mime_type !== parsed.data.mimeType)) return reply.code(409).send({ error: 'FILE_ID_CONFLICT' });
    const { error: uploadError } = await supabase.storage.from('cab-files').upload(parsed.data.objectPath, body, { contentType: parsed.data.mimeType, upsert: true });
    if (uploadError) throw uploadError;
    const { error: metadataError } = await supabase.from('files').upsert({ id: parsed.data.fileId, bucket: 'cab-files', object_path: parsed.data.objectPath, mime_type: parsed.data.mimeType, size_bytes: body.length, captured_at: parsed.data.capturedAt ?? null }, { onConflict: 'id' });
