...
const syncItemSchema = z.object({ type: z.enum(['SESSION_START', 'TRIP', 'FUEL', 'EXPENSE', 'SESSION_CLOSE']), payload: z.record(z.string(), z.unknown()) });
const syncSchema = z.object({ transactions: z.array(syncItemSchema).min(1).max(100) }).superRefine((value, ctx) => {
  const ids = value.transactions.map((item) => item.payload.clientTransactionId).filter((id): id is string => typeof id === 'string');
  const seen = new Set<string>();
  ids.forEach((id, index) => { if (seen.has(id)) ctx.addIssue({ code: 'custom', path: ['transactions', index, 'payload', 'clientTransactionId'], message: 'Duplicate client transaction ID in sync batch' }); seen.add(id); });
});
  ...
  app.post('/v1/sync', async (request, reply) => {
    ...
    for (const item of parsed.data.transactions) {
      ...
      try {
        ...
        switch (item.type) {
          case 'SESSION_START': { const body = startSessionSchema.parse(item.payload); result = await startSession({ ...body, driverId: identity.driverId, vehicleId: identity.vehicleId }); break; }
          case 'TRIP': { const body = tripSchema.parse(item.payload); result = await createTrip({ ...body, driverId: identity.driverId, vehicleId: identity.vehicleId }); break; }
          case 'FUEL': { const body = fuelSchema.parse(item.payload); result = await createFuel({ ...body, driverId: identity.driverId, vehicleId: identity.vehicleId }); break; }
          case 'EXPENSE': { const body = expenseSchema.parse(item.payload); result = await createExpense({ ...body, driverId: identity.driverId, vehicleId: identity.vehicleId }); break; }
          case 'SESSION_CLOSE': { const body = closeSessionSchema.parse(item.payload); result = await closeSession({ ...body, driverId: identity.driverId, vehicleId: identity.vehicleId }); break; }
        }
        results.push({ type: item.type, clientTransactionId, accepted: true, result });
      } catch (error) { results.push({ type: item.type, clientTransactionId, accepted: false, error: syncErrorCode(error) }); }
    }
