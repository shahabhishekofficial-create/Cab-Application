...
function throwRpcError(error: { message: string; code?: string | null }): never {
  const message = error.message || 'Supabase RPC failed';
  const err = new Error(message);
  err.name = error.code || 'SUPABASE_RPC_ERROR';
  throw err;
}

export async function startSession(input: StartSessionInput): Promise<Record<string, unknown>> {
...
export async function closeSession(input: CloseSessionInput): Promise<Record<string, unknown>> {
